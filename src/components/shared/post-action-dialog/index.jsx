...
export const PostActionDialog = () => {
  ...
  function onSubmit(values) {
    ...
    mutate(isEdit ? { id: data.id, formData } : { formData });
  }
            ...
            />
            {
              imagePreview ?
                <div className="relative h-24 w-fit mx-auto">
                  <img src={imagePreview} alt="Selected Image" className="h-full" />
                  <XIcon onClick={() => setImagePreview(null)} className="absolute right-0 top-0 cursor-pointer text-destructive" />
                </div>
                :
                <FormField
                  control={form.control}
