...
export const PostLike = ({ liked, postId }) => {
  ...
  useEffect(() => {
    if (data) setIsLiked(data?.liked ?? liked);
  }, [data]);

  return (
    <button disabled={isPending} onClick={handleLike} className="flex justify-center items-center gap-2 px-2 hover:bg-gray-50 rounded-full p-1">
      {
        isPending ?
          <Spinner />
          :
